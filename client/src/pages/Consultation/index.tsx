...
const Consultation = () => {
  ...
  const consultationTypes = [
    { value: 'critical-illness', label: '重疾咨询' },
    { value: 'chronic-disease', label: '慢病管理' },
    { value: 'health-asset', label: '健康资产规划' },
  ];
  ...
  return (
    <>
      ...
      <main className="flex-grow pt-16 sm:pt-20 pb-28 sm:pb-36 px-4 sm:px-6">
          ...
          <form className="space-y-12" onSubmit={handleFormSubmit}>
            <section className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <textarea rows={5} className="w-full bg-surface-container-lowest border-none rounded-2xl p-5 focus:ring-2 focus:ring-secondary/60 text-on-surface text-base font-body input-shadow resize-none" placeholder="请简要描述您的健康状况或当前需求,以便我们匹配专家..." value={formData.briefHistory} onChange={(e) => handleChange('briefHistory', e.target.value)} />
              </div>
            </section>
            <div className="pt-8">
              <button type="submit" className="w-full bg-primary text-on-primary rounded-full font-bold text-lg px-8 py-4 hover:bg-primary-container transition-all active:scale-95 shadow-lg flex items-center justify-center gap-2 touch-manipulation min-h-[56px]">
                提交咨询申请
