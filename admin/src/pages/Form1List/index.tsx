...
const Form1ListPage: React.FC = () => {
  const navigate = useNavigate();
  const [data, setData] = useState<Form1ListItem[]>([]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string) => (
        <Tag color={text === '病历咨询' ? 'blue' : 'green'}>{text}</Tag>
      ),
    },
    {
      title: '提交时间',
    ...
    },
    {
      title: '操作',
      width: 100,
      render: (_: unknown, record: Form1ListItem) => (
        <Button
          type="link"
          icon={<EyeOutlined />}
          onClick={() => navigate(`/form1/${record.id}`)}
        >
          详情
        </Button>
      ),
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 24 }}>咨询表单列表</h2>
      <Card>
